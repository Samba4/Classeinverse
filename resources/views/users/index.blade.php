...
@slot('title')
@lang('Gestion des etudiants (administrateurs en rouge)')
@endslot
    ...
    <table class="table table-dark text-white">
        ...
        <tbody style="text-align: justify">
            ...
            <tr @if($user->admin) style="color: red" @endif>
                ...
                <td>@if($user->email_verified_at){{ $user->email_verified_at->formatLocalized('%d %B %Y') }} à
                    {{ $user->email_verified_at->formatLocalized('%H:%M') }} @endif
                </td>
                ...
                <td>
                    <a type="button" href="{{ route('user.edit', $user->id) }}"
                        ...
                        Modifier") {{ $user->name }}"><i class="fas fa-edit fa-lg"></i></a>
                </td>
                <td>
                    @unless($user->admin)
                    <a type="button" href="{{ route('user.destroy', $user->id) }}"
                        class="btn btn-danger btn-sm pull-right invisible" data-toggle="tooltip" title="@lang("
                        Supprimer") {{ $user->name }}"><i class="fas fa-trash fa-lg"></i></a>
                    @endunless
                </td>
            </tr>
            @endforeach
